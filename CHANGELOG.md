...
### Added
- ไอคอนบนแท็บเบราว์เซอร์เป็นรูปกุญแจ SVG สีเขียวตามธีม (คง `favicon.ico` เดิมเป็นตัวสำรอง)
- ตั้ง `Referrer-Policy: strict-origin-when-cross-origin` ทุกหน้า — ลิงก์ออกไปเว็บอื่นส่งแค่ชื่อโดเมน
- หน้า **Docs** (`/docs`, ไอคอนหนังสือบนแถบบน) แสดง runbook · ดาวน์โหลดเป็นไฟล์สำหรับ LLM ได้ (`/llms.txt`, `/llms-full.txt`) · เปิดได้โดยไม่ต้องล็อกอิน

### Changed
- หน้าค้นโหลดทั้งคลังครั้งเดียว — ตัวกรองซ้าย/ไล่ดูทั้งคลังทำงานทันทีไม่ต้องรอ server · ยิง server เฉพาะตอนค้น และคำค้นเดิมไม่ยิงซ้ำ
- แท็บ Key/โปรเจกต์ใน Vault แสดงข้อมูลเดิมทันทีแล้วโหลดใหม่ข้างหลัง · ทุก API บอกเวลาใน header `Server-Timing`
- ปุ่ม "ขอ Key" บนการ์ด: เปิดเว็บเจ้าของ API พร้อมฟอร์มเพิ่ม Key ค้างไว้
