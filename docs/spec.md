...
- `GET /api/entries/:id` ให้หน้าแก้โหลดค่าเดิม
- หน้าจอ: ปุ่ม "เพิ่ม Entry" บน `/` → `/entries/new` · Entry `manual` มีป้าย "เพิ่มเอง" + ปุ่มแก้ (`/entries/:id`) และลบ (ยืนยันใน modal) บน EntryCard
